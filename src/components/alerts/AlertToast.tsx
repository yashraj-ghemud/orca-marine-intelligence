...
export function AlertToast({ alert, language }: { alert: MarineAlert; language: Language }) {
  ...
  return (
    <motion.div
      ...
      className="pointer-events-auto w-[330px] max-w-[calc(100vw-32px)] overflow-hidden break-words rounded-[10px] border border-ocean-danger-fg/30 bg-white shadow-[0_8px_28px_rgba(180,35,24,0.16)]"
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-t border-ocean-line/70 bg-white px-4 py-2.5">
...
export function AlertsPopover({ language }: { language: Language }) {
  ...
  return (
    <div className="orca-alerts-popover absolute right-0 top-[calc(100%+8px)] z-[800] flex w-[320px] flex-col overflow-hidden break-words rounded-[10px] border border-ocean-line bg-white shadow-[0_8px_28px_rgba(23,35,45,0.14)]">
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-ocean-line/80 bg-ocean-bg/60 px-3.5 py-2">
      ...
      {alerts.length === 0 ? (
        ...
        <ul className="orca-scroll min-h-0 max-h-[300px] overflow-y-auto overscroll-contain">
          {alerts.map((a) => (
