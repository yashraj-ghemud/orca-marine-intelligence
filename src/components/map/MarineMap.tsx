...
import { useMotionPreference } from "@/lib/intro";
import { getRegion, CYCLONE_RINGS_KM } from "@/lib/mock-marine-data";
import { circlePolygon, haversineKm } from "@/lib/map-utils";

/* Keep hooks inside MapContainer, including the always-visible vessel popup. */
function MarinePopup({ children }: { children: ReactNode }) {
  const map = useMap();
  ...
  const options = () => {
    ...
    return {
      autoPanPaddingTopLeft: L.point(narrow ? 12 : 250, Math.min(110, height * 0.2)),
      autoPanPaddingBottomRight: L.point(narrow ? 12 : 90, Math.min(270, height * 0.35)),
      maxWidth: Math.min(300, Math.max(210, width - 52)),
    };
  };
  useMapEvents({
    resize: () => {
      ...
      if (!popup) return;
      // React Leaflet does not update these options when Popup props change.
      L.setOptions(popup, options());
      if (popup.isOpen()) popup.update();
    },
  });
  return <Popup ref={popupRef} {...options()}>{children}</Popup>;
}
...
function MapCommandListener() {
  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      // A zero-size map produces an infinite bounds zoom and NaN coordinates.
      // Keep the newest command pending until its mobile tab becomes visible.
      if (!width || !height) { map.stop(); return; }
      ...
      if (delta.focus.kind === "position") {
        map.flyTo([delta.focus.position.lat, delta.focus.position.lng], delta.focus.zoom ?? map.getZoom(), { duration: 1.1, animate: !reduced });
      } else {
        const b = L.latLngBounds([
          [delta.focus.bounds[0].lat, delta.focus.bounds[0].lng],
          [delta.focus.bounds[1].lat, delta.focus.bounds[1].lng],
        ]);
        const [top, right, bottom, left] = delta.focus.padding ?? [90, 90, 90, 90];
        const sx = Math.min(1, width * 0.5 / Math.max(1, left + right));
        const sy = Math.min(1, height * 0.5 / Math.max(1, top + bottom));
        map.flyToBounds(b, {
          paddingTopLeft: [left * sx, top * sy],
          paddingBottomRight: [right * sx, bottom * sy],
          duration: 1.1, animate: !reduced, maxZoom: 10,
        });
      }
    };
    const onResize = () => {
